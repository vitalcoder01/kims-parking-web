...
import {useAppState, ParkingSlot, ParkingTask} from '../../../context/AppStateContext';
import {getSocket} from '../../../services/socket';
import {
  analyticsApi, AnalyticsPeriod, CommandCenterBundle, InsightCard, SlotClassification,
 ...
 *
 * Only renders on wide viewports (see App.tsx's useIsWide gate) — the rest
 * of this app is a deliberately mobile, phone-frame design (.phone-frame in
 * index.css), and that mobile experience is untouched: every one of these
 * sidebar sections other than "Dashboard" simply re-renders the SAME mobile
 * screen component the phone-frame admin already uses, just inside a wider
 * shell instead of degrading this into a second, parallel implementation.
 *
...
function periodDateRangeLabel(period: AnalyticsPeriod): string {
...
const linkBtnStyle: React.CSSProperties = {background: 'none', border: 'none', color: cc.accentBlue, fontSize: 11, fontWeight: 800, cursor: 'pointer', padding: 0};
...
function Donut({parts, size = 100, stroke = 13, centerLabel, centerSub}: {
...
function MiniSparkline({series, color, width = 60, height = 20}: {series: number[]; color: string; width?: number; height?: number}) {
...
function KpiCard({icon, iconColor, bg, value, label, deltaPct, onClick}: {
  icon: IconName; iconColor: string; bg: string; value: string; label: string; deltaPct: number | null; onClick: () => void;
}) {
  return (
...
function SlotsKpiCard({occPct, occupied, available, total, onClick}: {occPct: number; occupied: number; available: number; total: number; onClick: () => void}) {
  return (
...
function HealthKpiCard({health, onClick}: {health: OperationalHealth; onClick: () => void}) {
...
function TrendChart({days}: {days: {date: string; tasks: number; visitors: number}[]}) {
...
function Heatmap({heatmap}: {heatmap: DemandHeatmap}) {
...
function SlotUtilizationPanel({liveSlots, onViewAll}: {liveSlots: ParkingSlot[]; onViewAll: () => void}) {
...
function AiInsightsPanel({insights, onNavigate}: {insights: InsightCard[]; onNavigate: () => void}) {
  return (
...
function RealtimeOperationsPanel({tasks}: {tasks: ParkingTask[]}) {
...
function ParkingSlotMapPanel({liveSlots, classById, onOpenSlots}: {liveSlots: ParkingSlot[]; classById: Map<string, SlotClassification>; onOpenSlots: () => void}) {
              ...
              {b.slots.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => setSelected(on ? null : s.id)} title={s.id}
                    style={{
                      width: 24, height: 22, borderRadius: 5, border: on ? '1.5px solid #fff' : 'none',
...
function TaskFunnelPanel({funnelVolume}: {funnelVolume: TaskFunnelVolume}) {
...
function TopDriversPanel({drivers, onViewAll}: {drivers: DriverAnalytics[]; onViewAll: () => void}) {
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: 9}}>
          {active.map((d, i) => (
            <div key={d.id} style={{display: 'flex', alignItems: 'center', gap: 8}}>
              ...
              <span style={{fontSize: 10.5, fontWeight: 700, color: cc.textPrimary, width: 60, flexShrink: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis'}}>{d.name}</span>
...
function VisitorAnalyticsPanel({dailyCounts}: {dailyCounts: {date: string; count: number}[]}) {
...
function VisitorTypesPanel({byVehicleType, total, note}: {byVehicleType: Record<string, number>; total: number; note: string}) {
...
function AnomalyRadarPanel({categories}: {categories: AnomalyRadarCategory[]}) {
  return (
...
function AskParkingSystemPanel({bundle}: {bundle: CommandCenterBundle}) {
