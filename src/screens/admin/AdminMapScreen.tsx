...
export function AdminMapScreen() {
  ...
  const summaryCard = showingOwnerDetail && pickedSlot ? (
    <div style={{borderRadius: radius['2xl'], padding: 18, backgroundColor: colors.primary, boxShadow: `0 6px 14px ${colors.shadow}`}}>
  ...
  const legendItems: {bg: string; br: string; lbl: string}[] = [
  ...
  const blockGrid = blocks.length === 0 ? (
